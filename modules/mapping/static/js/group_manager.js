...
class GroupManager {
    ...
    renderGroupTable() {
        ...
        // グループが存在する場合、最初のグループを自動選択
        if (this.groups.length > 0) {
            const firstGroup = this.groups[0];
            const firstRow = tbody.querySelector('tr[data-group-id]');
            if (firstRow) {
                // 少し遅延して自動選択（DOM更新完了後に実行）
                setTimeout(() => {
                    console.log('初期グループを自動選択:', firstGroup.id);
                    this.selectGroup(firstGroup.id, firstRow);
                }, 100);
            }
        }
    }
    ...
    async updateGroup(groupId, row) {
        ...
        try {
            const response = await fetch(`/mapping/api/groups/${groupId}`, {
                method: 'PUT',
    ...
    async deleteGroup() {
        ...
        try {
            const response = await fetch(`/mapping/api/groups/${this.currentGroupId}`, {
                method: 'DELETE'
            });
